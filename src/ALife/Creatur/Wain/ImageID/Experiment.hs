------------------------------------------------------------------------
-- |
-- Module      :  ALife.Creatur.Wain.ImageID.Experiment
...
import qualified ALife.Creatur.Wain.Image.Object as O
...
import ALife.Creatur.Wain.ImageID.Action (Action(..), correct,
  correctActions)
import ALife.Creatur.Wain.Image.Pattern (Pattern, bigX)
import ALife.Creatur.Wain.Image.Tweaker (PatternTweaker(..))
import ALife.Creatur.Wain.Image.PatternDB (PatternDB, anyPattern)
import qualified ALife.Creatur.Wain.Image.Wain as IW
import qualified ALife.Creatur.Wain.ImageID.Universe as U
...
chooseObject freqs w db = do
  (img1, imageId1) <- evalStateT anyPattern db
  fromList $ zip [O.PObject img1 imageId1, O.AObject w] freqs
