...
trainOne :: (PatternWain, ModelCreationData) -> Object Action (ResponseTweaker Action) -> IO (PatternWain, ModelCreationData)
...
let Right (w, w') = DS.decode bs :: Either String (PatternWain, PatternWain)
let sgm = view (brain . classifier . patternMap) w
let numeral = head . show $ objectNum obj
let a = correctActions !! objectNum obj
